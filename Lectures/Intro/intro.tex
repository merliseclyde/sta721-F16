...
\item Instructor: Merlise Clyde   \\ 214 Old Chemistry \\
 Office Hours Tues/Thur 4:20-5:20 or by appointment
\item Teaching Assistant:  Chris Glynn
...
\item Introduce  R programming as needed
\end{itemize}
  \end{frame}
...
Mean function may be an intrinsicaly nonlinear function of $t$
  $$\E[Y_i] = f(t_i, \t)$$   
...
Quadratic in $x$, but linear in $\beta$'s
\end{frame}
...
\item For problems involving hypothesis testing or model selection
  frequentists and Bayesian methods can be strikingly different.
...
\item<6-> Jacobian of the
  transformation  $J(\Z \rightarrow \Y) = | \frac{\partial
    g}{\partial \Y} |$ 
\item<7>sugstitute  $g(\Y)$ for $\Z$ in density and
